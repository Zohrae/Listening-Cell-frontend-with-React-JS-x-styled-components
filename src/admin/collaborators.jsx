...
function Collaborators() {
  ...
  const currentItems = collaborators.slice(indexOfFirstItem, indexOfLastItem);

  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(collaborators.length / itemsPerPage); i++) {
    pageNumbers.push(i);
  }
  ...
  useEffect(() => {
    const fetchCollaborators = async () => {
      try {
        const response = await fetch(`http://localhost:8000/api/collaborateurs/`);
        if (response.ok) {
          const data = await response.json();
          const filteredCollaborators = data.filter(collaborator =>
            collaborator.nom.toLowerCase().includes(searchQuery.toLowerCase()) ||
            collaborator.prenom.toLowerCase().includes(searchQuery.toLowerCase()) ||
            collaborator.poste_occupe.toLowerCase().includes(searchQuery.toLowerCase())
          );
...
const handleNotificationClick = async () => {
  ...
  const handleRemove = async (collaboratorId) => {
    try {
      const response = await fetch(`http://localhost:8000/api/collaborateurs/${collaboratorId}/`, {
        method: 'DELETE',
      });
      if (response.ok) {
        // Remove the collaborator from the state
        setCollaborators(collaborators.filter(collaborator => collaborator.id !== collaboratorId));
      } else {
        console.error('Failed to delete collaborator:', response.statusText);
      }
    } catch (error) {
  ...
  const handleRemoveWithConfirmation = (collaboratorId) => {
    const confirmDelete = window.confirm("Voulez-vous vraiment supprimer ce collaborateur ?");
    if (confirmDelete) {
      handleRemove(collaboratorId);
    }
  };
  ...
  return (
    <html lang="en">
      ...
      <MenuIconContainer>
        ...
        <SideMenu>
            ...
            <SideMenuItemActive>
              <SideMenuItemLink href="#">
                ...
                <SidebarTitle show={sidebarOpen} style={{ color: 'var(--secondary-color)' }}>Collaborateurs</SidebarTitle>
              </SideMenuItemLink>
            </SideMenuItemActive>
        ...
        <CollabButton open={sidebarOpen}>
            Ajouter
          </CollabButton>
        <StyledTitle id="uniqueTitleId">Liste des Collaborateurs</StyledTitle>
            <BlueLine />
            ...
            <DataTable>
              ...
              <tbody>
              {currentItems.map(collaborator => (
                <tr key={collaborator.id}>
                  <LargeTableData>{collaborator.email}</LargeTableData>
                  <LargeTableData>{collaborator.nom}</LargeTableData>
                  <LargeTableData>{collaborator.prenom}</LargeTableData>
                  <LargeTableData>
                    <ActionButton onClick={() => handleRemoveWithConfirmation(collaborator.id)}>
